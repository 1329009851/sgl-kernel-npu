...
# Deepep Test — A5 服务器 deepep 端到端测试流程
...
| `A5_DOCKER` | 编译/安装/测试执行的容器名；空=宿主机直接跑 | `deepep-test-container` |
| `A5_USE_PROXY` | 编译前是否注入代理（自动拉 catlass 等）；`true`/`false` | `true` |
| `A5_HTTP_PROXY` | 代理地址 | `http://proxy.example.com:port` |
| `A5_NO_PROXY` | 代理排除列表 | `127.0.0.1,*.example.com,...` |
...
```bash
bash .opencode/skills/deepep-test/scripts/setup-ssh.sh
```
...
```bash
bash scripts/flow.sh [--num-process N] [--skip-pack] [--skip-build]
```
...
| `flow.sh` | 一键全流程 | `--num-process` `--skip-pack` `--skip-build` |
